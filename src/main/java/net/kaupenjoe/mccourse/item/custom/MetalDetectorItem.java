...
public class MetalDetectorItem extends Item {
	...
	private void spawnFoundParticles(ItemUsageContext context, BlockPos pos, BlockState blockState) {
		for (int i = 0; i < 20; i++) {
			ServerWorld world = ((ServerWorld) context.getWorld());
			world.spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, blockState),
				pos.getX() + 0.5d, pos.getY() + 1, pos.getZ() + 0.5d, 2,
				Math.cos(i) * 0.25d, 0.15d, Math.sin(i) * 0.25d, 5f);
		}
	}
	...
	private void addNbtDataToDataTablet(PlayerEntity player, BlockPos position, Block block) {
		...
		nbtData.putString("mccourse.last_valuable_found",
			"Valuable Found: " + block.getName().getString() + " at " +
