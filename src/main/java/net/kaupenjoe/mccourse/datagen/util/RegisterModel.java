...
public final class RegisterModel {
	 ...
	 * Register a bow item
	 */
	public static void registerBow(ItemModelGenerator generator, Item item) {
		// Submodels ("_pulling_0", "_pulling_1", "_pulling_2")
		Model m = new Model(Optional.of(ModelIds.getItemModelId(item)), Optional.empty(), TextureKey.LAYER0);
		for (int i = 0; i < 3; i++) {
			generator.register(item, "_pulling_" + i, m);
		}
		// Model (default)
		Models.GENERATED.upload(ModelIds.getItemModelId(item), TextureMap.layer0(item),
			generator.writer, CreateExtraJson::createBowJson);
	}
}
