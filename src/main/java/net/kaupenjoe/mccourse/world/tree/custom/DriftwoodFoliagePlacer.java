...
public class DriftwoodFoliagePlacer extends FoliagePlacer {
	public static final Codec<DriftwoodFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) ->
		fillFoliagePlacerFields(instance)
			.and(Codec.intRange(0, 12).fieldOf("height").forGetter((placer) ->
				placer.height))
			.apply(instance, DriftwoodFoliagePlacer::new));
	...
	protected void generate(TestableWorld world, BlockPlacer placer, Random random,
		...
		for (int i = 0; i < 4; i++) {
			...
			this.generateSquare(world, placer, random, config, treeNode.getCenter().up(i),
				2, i + 1, treeNode.isGiantTrunk());
		}
	}
